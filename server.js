...
app.post('/api/contact', async (req, res) => {
  ...
  const htmlBody = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <div style="padding: 24px; background: #f9f9f9;">
        <h3 style="color: rgba(60, 77, 95, 1); margin-top: 0;">Contact Details</h3>
        <table style="width: 100%; border-collapse: collapse;">
          ...
          <tr><td style="padding: 8px; font-weight: bold; color: #555;">Interest:</td><td style="padding: 8px;">${interest || 'Not specified'}</td></tr>
        </table>
      </div>
      <div style="padding: 16px; text-align: center; color: #999; font-size: 12px;">
        Sent from Love Is Blinds DFW Landing Page
      </div>
    </div>
  `;
  ...
  try {
    ...
    res.status(500).json({ error: 'Failed to send your request. Please try again or call us directly.' });
  }
});

app.listen(PORT, () => {
  console.log(`Love Is Blinds server running at http://localhost:${PORT}`);
});
